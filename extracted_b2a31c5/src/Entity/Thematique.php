...
class Thematique
{
    ...
    #[Assert\Length(max: 50, maxMessage: 'Le code ne peut pas d├®passer {{ limit }} caract├¿res.')]
    ...
    #[Assert\Regex(pattern: '/^#([A-Fa-f0-9]{3}|[A-Fa-f0-9]{6})$/u', message: 'La couleur doit ├¬tre un code hexad├®cimal (ex. #A7C7E7 ou #F00).')]
    private ?string $couleur = null;

    /** Chemin de l'image repr├®sentant la th├®matique (upload) */
    ...
    #[Assert\Length(max: 255, maxMessage: 'Le sous-titre ne peut pas d├®passer {{ limit }} caract├¿res.')]
    private ?string $sousTitre = null;

    #[ORM\Column(type: 'smallint', nullable: true)]
    #[Assert\Range(min: 0, max: 32767, notInRangeMessage: 'L\'ordre doit ├¬tre entre {{ min }} et {{ max }}.')]
    private ?int $ordre = null;

    #[ORM\Column(type: 'boolean', options: ['default' => true])]
    private bool $actif = true;

    #[ORM\Column(type: 'string', enumType: PublicCible::class, columnDefinition: "ENUM('Enfant', 'Parent', 'M├®decin', '├ëducateur', 'Aidant', 'Autre')", nullable: true)]
    #[Assert\NotNull(message: 'Veuillez choisir un public cible.')]
    private ?PublicCible $publicCible = null;

    #[ORM\Column(type: 'string', enumType: NiveauDifficulte::class, columnDefinition: "ENUM('D├®butant', 'Interm├®diaire', 'Avanc├®')", nullable: true)]
    #[Assert\NotNull(message: 'Veuillez choisir un niveau de difficult├®.')]
