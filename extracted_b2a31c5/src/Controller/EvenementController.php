...
final class EvenementController extends AbstractController
{
    ...
    public function new(Request $request): Response
    {
        ...
        if ($form->isSubmitted() && $form->isValid()) {
            ...
            $this->addFlash('success', 'L\'├®v├®nement a ├®t├® cr├®├® avec succ├¿s.');
    ...
    public function show(Request $request, int $id): Response
    {
        ...
        if ($evenement === null) {
            throw new NotFoundHttpException('├ëv├®nement introuvable.');
        }
    ...
    public function replyMessage(Request $request, int $id): Response
    {
        ...
        if (!$this->isCsrfTokenValid('admin_event_reply_' . $id . '_' . $userId, (string) $request->request->get('_token'))) {
            $this->addFlash('error', 'Jeton de s├®curit├® invalide.');
        ...
        if ($form->isSubmitted() && $form->isValid()) {
            ...
            $this->entityManager->flush();
            $this->addFlash('success', 'Votre r├®ponse a ├®t├® envoy├®e.');
        } else {
            $this->addFlash('error', 'Le message ne peut pas ├¬tre vide.');
        }
    ...
    public function deleteMessage(Request $request, int $messageId): Response
    {
        ...
        $this->entityManager->flush();
        $this->addFlash('success', 'Le message a ├®t├® supprim├®.');
        return $this->redirectToRoute('admin_evenement_show', ['id' => $evenement->getId(), 'user' => $userId]);
    }
    ...
    public function editMessage(Request $request, int $messageId): Response
    {
        ...
        if ($message->getEnvoyePar() !== MessageEvenement::ENVOYE_PAR_ADMIN) {
            $this->addFlash('error', 'Seuls les messages envoy├®s par l\'admin peuvent ├¬tre modifi├®s.');
        ...
        if ($form->isSubmitted()) {
            ...
            if ($form->isValid()) {
                $this->entityManager->flush();
                $this->addFlash('success', 'Le message a ├®t├® modifi├®.');
                return $this->redirectToRoute('admin_evenement_show', ['id' => $evenement->getId(), 'user' => $userId]);
            }
        }
    ...
    public function sendReminders(Request $request, int $id): Response
    {
        ...
        $msg = sprintf(
            'Rappels envoy├®s : %d e-mail(s).',
            $result['sentEmail']
        );
        if ($result['sentEmail'] === 0 && $result['sentSms'] === 0) {
            $this->addFlash('warning', 'Aucun rappel envoy├®. V├®rifiez que lÔÇÖ├®v├®nement a des participants avec le statut ┬½ Accept├® ┬╗ ou ┬½ En attente ┬╗ et une adresse e-mail renseign├®e dans leur profil.');
    ...
    public function generateImage(Request $request, int $id): JsonResponse
    {
        $evenement = $this->evenementRepository->find($id);
        if ($evenement === null) {
            return new JsonResponse(['error' => '├ëv├®nement introuvable.'], Response::HTTP_NOT_FOUND);
        }
        if (!$this->isCsrfTokenValid('admin_evenement_generate_image_' . $id, (string) $request->request->get('_token'))) {
            return new JsonResponse(['error' => 'Jeton de s├®curit├® invalide.'], Response::HTTP_FORBIDDEN);
        }
        ...
        if ($customPrompt !== '') {
            ...
            if ($evenement->getThematique() !== null) {
                $parts[] = 'Th├®matique : ' . $evenement->getThematique()->getNomThematique();
            }
            ...
            if ($prompt === '') {
                return new JsonResponse(['error' => 'Saisissez un prompt ou ajoutez au moins un titre / une description ├á l\'├®v├®nement.'], Response::HTTP_BAD_REQUEST);
            }
            $prompt = 'Affiche pour un ├®v├®nement bienveillant, familles et inclusion. ' . $prompt . ' Style illustration douce, couleurs apaisantes, professionnel.';
        }
        ...
        if ($result === null) {
            ...
            $msg = 'La g├®n├®ration n\'a pas abouti. R├®essayez dans un instant.';
            if ($detail !== '') {
                $msg .= ' (' . $detail . ')';
            }
    ...
    public function participantsPdf(int $id): Response
    {
        ...
        if (!class_exists(\Dompdf\Dompdf::class)) {
            $this->addFlash('error', 'Export PDF indisponible : installez "composer require dompdf/dompdf" puis r├®essayez.');
            return $this->redirectToRoute('admin_evenement_show', ['id' => $id]);
        }
    ...
    public function acceptInscription(Request $request, int $inscriptionId): Response
    {
        ...
        $this->entityManager->flush();
        $this->addFlash('success', 'Inscription accept├®e.');
        return $this->redirectToRoute('admin_evenement_show', ['id' => $inscription->getEvenement()->getId()]);
    }
    ...
    public function refuseInscription(Request $request, int $inscriptionId): Response
    {
        ...
        $this->entityManager->flush();
        $this->addFlash('success', 'Inscription refus├®e.');
        return $this->redirectToRoute('admin_evenement_show', ['id' => $inscription->getEvenement()->getId()]);
    }
    ...
    public function edit(Request $request, int $id): Response
    {
        ...
        if ($form->isSubmitted() && $form->isValid()) {
            $this->entityManager->flush();
            $this->addFlash('success', 'L\'├®v├®nement a ├®t├® modifi├® avec succ├¿s.');
    ...
    public function delete(Request $request, int $id): Response
    {
        ...
        $this->entityManager->flush();
        $this->addFlash('success', 'L\'├®v├®nement a ├®t├® supprim├®.');
        return $this->redirectToRoute('admin_evenement_index');
    }
    ...
    public function geocode(Request $request, NominatimGeocodingService $geocoding): JsonResponse
    {
        ...
        if (!is_string($address) || trim($address) === '') {
            return new JsonResponse(['error' => 'Param├¿tre address requis.'], Response::HTTP_BAD_REQUEST);
        }
    ...
    /**
     * G├®n├¿re un lien Zoom sans ├®v├®nement (pour formulaire de cr├®ation).
    ...
    public function genererZoomLien(Request $request): JsonResponse
    {
        ...
        if (!\in_array($mode, ['en_ligne', 'hybride'], true)) {
            return new JsonResponse(['error' => 'S├®lectionnez le mode ┬½ En ligne ┬╗ ou ┬½ Hybride ┬╗.'], Response::HTTP_BAD_REQUEST);
        }
        $title = \is_string($request->request->get('title')) ? trim($request->request->get('title')) : '';
        if ($title === '') {
            return new JsonResponse(['error' => 'Saisissez un titre pour l\'├®v├®nement.'], Response::HTTP_BAD_REQUEST);
        }
        ...
        if ($dateStr === '' || $heureDebutStr === '' || $heureFinStr === '') {
            return new JsonResponse(['error' => 'Date et horaires (d├®but, fin) requis.'], Response::HTTP_BAD_REQUEST);
        }
        try {
            ...
            return new JsonResponse(['error' => 'Date ou heure de d├®but invalide.'], Response::HTTP_BAD_REQUEST);
        }
        try {
    ...
    public function genererZoom(Request $request, int $id): JsonResponse
    {
        ...
        // Utiliser le mode envoy├® par le formulaire (changement Pr├®sentiel ÔåÆ En ligne / Hybride sans enregistrer)
        ...
        if ($mode !== 'en_ligne' && $mode !== 'hybride') {
            return new JsonResponse(['error' => 'Le lien Zoom est r├®serv├® aux ├®v├®nements en ligne ou hybrides. S├®lectionnez ┬½ En ligne ┬╗ ou ┬½ Hybride ┬╗ et r├®essayez.'], Response::HTTP_BAD_REQUEST);
        }
        ...
        if ($dateEvent === null || $heureDebut === null || $heureFin === null) {
            return new JsonResponse(['error' => 'Date et horaires requis pour cr├®er la r├®union Zoom.'], Response::HTTP_BAD_REQUEST);
        }
        $startTime = new \DateTime(
