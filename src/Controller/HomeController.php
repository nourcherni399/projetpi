...
final class HomeController extends AbstractController
{
    ...
    public function appointments(): Response
    {
        ...
        if ($user instanceof User) {
            foreach ($medecins as $m) {
                ...
                $user_ratings[$m->getId()] = $r !== null ? $r->getNote() : null;
            }
        }
        ...
        if ($disponibiliteId !== null && $disponibiliteId !== '' && $dateRdvStr !== '') {
            foreach ($slots as $s) {
                if ((int) $s['disponibilite_id'] === (int) $disponibiliteId && $s['date_rdv'] === $dateRdvStr) {
                    $selectedSlotAvailable = $s['available'] ?? true;
                    break;
                }
            }
        }
    ...
    /**
     * @return list<array{disponibilite_id: int, date_rdv: string, label: string, day_name?: string, time_range?: string, available?: bool}>
     */
    private function getAvailableSlotsForMedecin(Medcin $medecin): array
    {
        ...
        foreach ($dispos as $dispo) {
            ...
            // Handle both date-based and recurrence-based disponibilities
            if ($dispo->getDate() !== null) {
                ...
                $taken = $this->rendezVousRepository->isSlotTaken($dispo, $dateImmutable);
            ...
            } elseif ($dispo->getJour() !== null) {
                // Recurrence-based availability (by day of week)
                $jourValue = $dispo->getJour()->value;
                $targetDayNum = self::JOUR_TO_NUMBER[$jourValue] ?? null;
        ...
        // Keep only one occurrence per recurring slot (closest date)
        $slotByRecurrence = [];
        foreach ($slots as $s) {
            $pos = strrpos($s['label'], ', ');
            if ($pos === false) {
