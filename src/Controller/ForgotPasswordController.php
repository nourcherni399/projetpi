...
final class ForgotPasswordController extends AbstractController
{
    ...
    #[Route('/verifier-pin', name: 'verify_pin', methods: ['GET', 'POST'])]
    public function verifyPin(Request $request): Response
    {
        ...
        if ($request->isMethod('POST')) {
            if ($email === '' || $pin === '') {
                ...
                $user = $this->userRepository->findOneBy(['email' => $email]);
                if (!$user instanceof User) {
                    $this->addFlash('error', 'Adresse e-mail incorrect.');
                    ...
                    // PIN valide, rediriger vers la page de nouveau mot de passe
            ...
            return $this->redirectToRoute('app_forgot_password_verify_pin', ['email' => $email, 'pin' => $pin]);
        }

        return $this->render('front/auth/forgot_password_pin.html.twig', [
            'email' => $email,
            'pin' => $pin,
        ]);
    }

    #[Route('/nouveau-mot-de-passe', name: 'new', methods: ['GET', 'POST'])]
    public function new_password(Request $request): Response
    {
        if ($this->getUser()) {
            return $this->redirectToRoute('home');
        }
        ...
        $user = $this->userRepository->findOneBy(['email' => $email]);
        if (!$user instanceof User || $user->getResetPin() !== $pin || $user->getResetPinExpiresAt() === null || $user->getResetPinExpiresAt() < new \DateTimeImmutable()) {
            $this->addFlash('error', 'Code PIN invalide ou expiré. Veuillez recommencer le processus.');
            return $this->redirectToRoute('app_forgot_password_request');
        }
        ...
        if ($request->isMethod('POST')) {
            ...
            $passwordErrors = $this->validatePasswordRules((string) ($newPassword ?? ''));
            if ($passwordErrors !== []) {
                foreach ($passwordErrors as $errorMessage) {
            ...
            return $this->redirectToRoute('app_forgot_password_new', ['email' => $email, 'pin' => $pin]);
        }

        return $this->render('front/auth/forgot_password_new.html.twig', [
            'email' => $email,
            'pin' => $pin,
        ]);
    }
    ...
    {
        // Rediriger vers la nouvelle méthode de flux
        return $this->redirectToRoute('app_forgot_password_request');
    }
    ...
    {
        $verifyPinUrl = $this->urlGenerator->generate('app_forgot_password_verify_pin', [
            'email' => $toEmail,
        ...
        $email = (new Email())
            ...
            ->html($this->getEmailHtml($pin, $verifyPinUrl));
        $this->mailer->send($email);
    }

    private function getEmailHtml(string $pin, string $verifyPinUrl): string
    {
        $verifyPinUrl = htmlspecialchars($verifyPinUrl, \ENT_QUOTES, 'UTF-8');
        ...
        <p><strong>Cliquez sur le bouton ci-dessous</strong> pour saisir ce code PIN :</p>
        <p style="text-align:center;"><a href="{$verifyPinUrl}" class="btn">Saisir le code PIN</a></p>
        <p>Si le bouton ne fonctionne pas, copiez ce lien dans votre navigateur :<br><a href="{$verifyPinUrl}">{$verifyPinUrl}</a></p>
     ...
     * @return list<string>
     */
    private function validatePasswordRules(string $password): array
    {
        $errors = [];
